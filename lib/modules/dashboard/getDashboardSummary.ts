...
export async function getDashboardSummary(userId: string): Promise<DashboardSummary> {
  ...
  // 1) Cargar datos base (SIN status, porque no existe en tu schema)
  const [accountsRes, monthTxRes, tx30Res, categoriesRes] = await Promise.all([
    supabase.from("accounts").select("id, name, balance").eq("user_id", userId),
    supabase
      .from("transactions")
      .select("id, created_at, amount, type, category_id, description, account_id")
      .eq("user_id", userId)
      .gte("created_at", monthStart),
    supabase
      .from("transactions")
      .select("created_at, amount, type, category_id")
      .eq("user_id", userId)
  ...
  // En tu schema no hay status => asumimos todo "posted"
  const monthPosted = monthTx;
  ...
  // 2) budgets (opcional): si la tabla no existe o no hay policy, NO tumbes el dashboard
  let budgetProgress: DashboardSummary["budgetProgress"] = [];
  try {
    const budgetLimitsRes = await supabase
      .from("budget_limits")
      .select("category_id, limit_amount")
      .eq("user_id", userId);

    // Si falla por tabla/policy, lo ignoramos (lo arreglamos después)
    if (!budgetLimitsRes.error) {
      ...
      budgetProgress = budgetLimits.map((bl: any) => {
